...
(() => {
    ...
    async function loadChat(){const{data,error}=await supabaseClient.from("chat_mensagens").select("id,live_id,aluno_id,mensagem,created_at,autor:usuarios!chat_mensagens_aluno_id_fkey(nome,foto_url,perfil)").eq("live_id",state.live.id).order("created_at",{ascending:true}).limit(300);if(error)throw error;$("chatMessages").replaceChildren();state.chatIds.clear();(data||[]).forEach(message=>addMessage(message));if(!(data||[]).length)showChatEmpty();}
    ...
    async function completeAuthor(message){if(String(message.aluno_id)===String(state.usuario.id))return{...message,autor:state.usuario};const{data}=await supabaseClient.from("usuarios").select("nome,foto_url,perfil").eq("id",message.aluno_id).maybeSingle();return{...message,autor:data};}
